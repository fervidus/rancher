plan rancher::install_rancher(
  ...
  $login_token_result_set = run_task('rancher::get_login_token', 'localhost', control_plane => $server)
  $login_token = $login_token_result_set.first().message
  out::message("*** login_token: ${login_token}")

  $api_token_result_set = run_task('rancher::get_api_key', 'localhost', control_plane => $server, login_token => $login_token)
  ...
  $cluster_join_result_set = run_task('rancher::get_cluster_registration_token', 'localhost', control_plane => $server, cluster_id => $cluster_id, api_token => $api_token)
