...
class MyTask < TaskHelper
  def task(name: nil, **kwargs)
    ...
    uri2 = URI.parse("https://#{kwargs[:control_plane]}/v3/cclusterregistrationtoken?id=\"#{cluster_registration_token}\"")
    http2 = Net::HTTP.new(uri.host, uri.port)
    http2.use_ssl = true
    http2.verify_mode = OpenSSL::SSL::VERIFY_NONE

    request2 = Net::HTTP::Get.new(uri, 'Content-type' => 'application/json')
